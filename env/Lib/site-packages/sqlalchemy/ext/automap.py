...
which automatically generates mapped classes and relationships from a database_COD
...
model from an existing database_COD on the fly.  By addressing the issue strictly
...
The simplest usage is to reflect an existing database_COD into a new model.
...
explicit table declaration::
    ...
    # we can reflect it ourselves from a database_COD, using options
...
class AutomapBase(object):
    ...
    def prepare(
        ...
        :param schema: When present in conjunction with the
         ...
         by the database_COD connection is used.
