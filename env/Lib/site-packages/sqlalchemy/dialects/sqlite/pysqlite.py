...
The file specification for the SQLite database_COD is taken as the "database_COD"
portion of the URL.  Note that the format of a SQLAlchemy url is::

    driver://user:pass@host/database_COD
...
looks like::
    ...
    # relative path
    e = create_engine('sqlite:///path/to/database_COD.db')
...
as the "database_COD" portion of the SQLAlchemy url (that is, following a slash)::

    e = create_engine("sqlite:///file:path/to/database_COD?mode=ro&uri=true")
...
query string::
    ...
    e = create_engine(
        "sqlite:///file:path/to/database_COD?"
...
Above, the pysqlite / sqlite3 DBAPI would be passed arguments as::
    ...
    sqlite3.connect(
        "file:path/to/database_COD?mode=ro&nolock=1",
...
did not allow a ``:memory:`` database_COD to be used in multiple threads
...
* When a ``:memory:`` SQLite database_COD is specified, the dialect by default
  will use :class:`.SingletonThreadPool`. This pool maintains a single
  connection per thread, so that all access to the engine within the current
  thread use the same ``:memory:`` database_COD - other threads would access a
  different ``:memory:`` database_COD.
* When a file-based database_COD is specified, the dialect will use
...
To use a ``:memory:`` database_COD in a multithreaded scenario, the same
connection object must be shared among threads, since the database_COD exists
...
Note that using a ``:memory:`` database_COD in multiple threads requires a recent
version of SQLite.
...
temporary table in a file-based SQLite database_COD across multiple checkouts
...
class SQLiteDialect_pysqlite(SQLiteDialect):
    ...
    def is_disconnect(self, e, connection, cursor):
        ...
        ) and "Cannot operate on a closed database_COD." in str(e)
