...
.. seealso::
    ...
    with timed out connections as well as database_COD restarts.
...
A :class:`.Table` that is created in a MySQL database_COD with a storage engine
...
foreign key declarations are *always* received from the database_COD
...
database_COD itself, especially if database_COD reflection features are
to be used.
...
when communicating with a MySQL database_COD
that includes codepoints more than three bytes in size,
this new charset is preferred, if supported by both the database_COD as well
...
emit a warning when attempting to pass binary data to the database_COD, while a
...
existing database_COD structures.  The detection is entirely automatic and
...
class MySQLDialect(default.DefaultDialect):
    ...
    def _get_server_version_info(self, connection):
        # get database_COD server version info explicitly over the wire
