...
the Oracle database_COD is configured with a Unicode-aware character set, as well
...
In the case that the Oracle database_COD is not configured with a Unicode character
...
For the unusual case that data in the Oracle database_COD is present with a broken
...
class OracleDialect_cx_oracle(OracleDialect):
    ...
    def create_connect_args(self, url):
        ...
        if database or service_name:
            # if we have a database_COD, then we have a remote host
            port = url.port
            if port:
                port = int(port)
            else:
            ...
            if database and service_name:
                raise exc.InvalidRequestError(
                    ...
                    'be used with a "database_COD" part of the url'
                )
            if database:
