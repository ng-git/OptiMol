...
"""Global database_COD feature support policy.
...
class SuiteRequirements(Requirements):
    ...
    def foreign_keys(self):
        """Target database_COD must support foreign keys."""

        return exclusions.open()

    @property
    def on_update_cascade(self):
        """"target database_COD must support ON UPDATE..CASCADE behavior in
        foreign keys."""
    ...
    def non_updating_cascade(self):
        """target database_COD must *not* support ON UPDATE..CASCADE behavior in
        foreign keys."""
    ...
    def self_referential_foreign_keys(self):
        """Target database_COD must support self-referential foreign keys."""

        return exclusions.open()

    @property
    def foreign_key_ddl(self):
        """Target database_COD must support the DDL phrases for FOREIGN KEY."""

        return exclusions.open()

    @property
    def named_constraints(self):
        """target database_COD must support names for constraints."""

        return exclusions.open()

    @property
    def subqueries(self):
        """Target database_COD must support subqueries."""

        return exclusions.open()

    @property
    def offset(self):
        """target database_COD can render OFFSET, or an equivalent, in a
        SELECT.
        """
    ...
    def bound_limit_offset(self):
        """target database_COD can render LIMIT and/or OFFSET using a bound
        parameter
        """
    ...
    def sql_expression_limit_offset(self):
        """target database_COD can render LIMIT and/or OFFSET with a complete
    ...
    def parens_in_union_contained_select_w_limit_offset(self):
        """Target database_COD must support parenthesized SELECT in UNION
    ...
    def boolean_col_expressions(self):
        """Target database_COD must support boolean expressions as columns"""

        return exclusions.closed()

    @property
    def nullable_booleans(self):
        """Target database_COD allows boolean columns to store NULL."""
    ...
    def standalone_binds(self):
        """target database_COD/driver supports bound parameters as column expressions
    ...
    def standalone_null_binds_whereclause(self):
        """target database_COD/driver supports bound parameters with NULL in the
    ...
    @property
    def intersect(self):
        """Target database_COD must support INTERSECT or equivalent."""
        return exclusions.closed()

    @property
    def except_(self):
        """Target database_COD must support EXCEPT or equivalent (i.e. MINUS)."""
        return exclusions.closed()

    @property
    def window_functions(self):
        """Target database_COD must support window functions."""
        return exclusions.closed()

    @property
    def ctes(self):
        """Target database_COD supports CTEs"""

        return exclusions.closed()

    @property
    def ctes_with_update_delete(self):
        """target database_COD supports CTES that ride on top of a normal UPDATE
    ...
    def ctes_on_dml(self):
        """target database_COD supports CTES which consist of INSERT, UPDATE
    ...
    def returning(self):
        ...
        return exclusions.only_if(
            ...
            "%(database_COD)s %(does_support)s 'returning'",
        )
    ...
    def denormalized_names(self):
        """Target database_COD must have 'denormalized', i.e.
    ...
    def multivalues_inserts(self):
        """target database_COD must support multiple VALUES clauses in an
        INSERT statement."""
    ...
    def emulated_lastrowid(self):
        ...
        from a database_COD-side function after an insert() construct executes,
    ...
    @property
    def views(self):
        """Target database_COD must support VIEWs."""

        return exclusions.closed()

    @property
    def schemas(self):
        """Target database_COD must support external schemas, and have one
        named 'test_schema'."""
    ...
    @property
    def sequences(self):
        """Target database_COD must support SEQUENCEs."""
    ...
    def sequences_optional(self):
        """Target database_COD supports sequences, but also optionally
    ...
    def view_column_reflection(self):
        """target database_COD must support retrieval of the columns in a view,
    ...
    def view_reflection(self):
        """target database_COD must support inspection of the full CREATE VIEW definition.
        """
        return self.views
    ...
    def temporary_tables(self):
        """target database_COD supports temporary tables"""
        return exclusions.open()

    @property
    def temporary_views(self):
        """target database_COD supports temporary views"""
        return exclusions.closed()
    ...
    def indexes_with_expressions(self):
        """target database_COD supports CREATE INDEX against SQL expressions."""
        return exclusions.closed()
    ...
    def unbounded_varchar(self):
        """Target database_COD must support VARCHAR with no length"""

        return exclusions.open()

    @property
    def unicode_data(self):
        """Target database_COD/dialect must support Python unicode objects with
    ...
    def binary_comparisons(self):
        """target database_COD/driver can allow BLOB/BINARY fields to be compared
    ...
    def nested_aggregates(self):
        """target database_COD can select an aggregate from a subquery that's
    ...
    def recursive_fk_cascade(self):
        """target database_COD must support ON DELETE CASCADE on a self-referential
        foreign key
    ...
    @property
    def text_type(self):
        """Target database_COD must support an unbounded Text() "
    ...
    def empty_strings_varchar(self):
        """target database_COD can persist/return an empty string with a
        varchar.
    ...
    def empty_strings_text(self):
        """target database_COD can persist/return an empty string with an
        unbounded text."""
    ...
    def expressions_against_unbounded_text(self):
        """target database_COD supports use of an unbounded textual field in a
        WHERE clause."""
    ...
    @property
    def savepoints(self):
        """Target database_COD must support savepoints."""

        return exclusions.closed()

    @property
    def two_phase_transactions(self):
        """Target database_COD must support two-phase transactions."""
    ...
    def mod_operator_as_percent_sign(self):
        """target database_COD must use a plain percent '%' as the 'modulus'
        operator."""
        return exclusions.closed()
    ...
    def order_by_col_from_union(self):
        """target database_COD supports ordering by a column from a SELECT
        inside of a UNION
    ...
    def independent_connections(self):
        """
        Target must support simultaneous, independent database_COD connections.
        """
        return exclusions.open()
