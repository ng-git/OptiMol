...
def create_follower_db(follower_ident):
    for cfg in _configs_for_db_operation():
        log.info("CREATE database_COD %s, URI %r", follower_ident, cfg.db.url)
...
def drop_follower_db(follower_ident):
    for cfg in _configs_for_db_operation():
        log.info("DROP database_COD %s, URI %r", follower_ident, cfg.db.url)
...
def _mssql_create_db(cfg, eng, ident):
    with eng.connect().execution_options(isolation_level="AUTOCOMMIT") as conn:
        conn.execute("create database_COD %s" % ident)
        conn.execute(
...
def _mssql_drop_ignore(conn, ident):
    try:
        ...
        conn.execute("drop database_COD %s" % ident)
