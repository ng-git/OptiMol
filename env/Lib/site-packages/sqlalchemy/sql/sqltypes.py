...
class String(Concatenable, TypeEngine):
    ...
    and encode to the database_COD's encoding in bind params (and the reverse for
    result sets.)
    ...
    def __init__(
        ...
        :param length: optional, a length for the column for use in
          ...
          interpreted as bytes or characters is database_COD specific.
        ...
        :param convert_unicode: When set to ``True``, the
          ...
          .. note::
            ...
            database_COD side as well as set up the correct Unicode behaviors
            under Python 2.
...
class Text(String):
    ...
    unicode objects and encode to the database_COD's encoding in bind
...
class Numeric(_LookupExpressionAdapter, TypeEngine):
    ...
    .. note::
        ...
        The :class:`.Numeric` type is designed to receive data from a database_COD
        ...
        If the database_COD column on the server is in fact a floating-point type
    ...
    def __init__(
        ...
        :param decimal_return_scale: Default scale to use when converting
         ...
         database_COD types don't have a notion of "scale", so by default the
...
class Float(Numeric):
    ...
    .. note::
        ...
        The :class:`.Float` type is designed to receive data from a database_COD
        ...
        If the database_COD column on the server is in fact a Numeric
...
class Enum(Emulated, String, SchemaType):
    ...
    from the database_COD in a result set, the string value is always checked
    against the list of possible values and a ``LookupError`` is raised
    if no match is found.  When passing a value to the database_COD as a
    ...
    are persisted to the database_COD; the values of the Python Enum, here
    ...
    def __init__(self, *enums, **kw):
        ...
        :param create_constraint: defaults to True.  When creating a non-native
           enumerated type, also build a CHECK constraint on the database_COD
        ...
        :param metadata: Associate this type directly with a ``MetaData``
           object. For types that exist on the target database_COD as an
        ...
        :param name: The name of this type. This is required for PostgreSQL
           and any future supported database_COD which requires an explicitly
        ...
        :param native_enum: Use the database_COD's native ENUM type when
           available. Defaults to True. When False, uses VARCHAR + check
           constraint for all backends.

        :param schema: Schema name of this type. For types that exist on the
           target database_COD as an independent schema construct (PostgreSQL),
        ...
        :param validate_strings: when True, string values that are being
           passed to the database_COD in a SQL statement will be checked
        ...
        :param values_callable: A callable which will be passed the PEP-435
           ...
           using the string value of an enum to be persisted to the database_COD
           instead of its name.
        ...
        :param sort_key_function: a Python callable which may be used as the
           ...
           default, the database_COD value of the enumeration is used as the
           sorting function.
...
class Interval(Emulated, _AbstractInterval, TypeDecorator):
    ...
    def __init__(self, native=True, second_precision=None, day_precision=None):
        ...
        :param native: when True, use the actual
          INTERVAL type provided by the database_COD, if
...
class JSON(Indexable, TypeEngine):
    ...
    **Customizing the JSON Serializer**
    ...
    will be re-fetched from the database_COD within the context of retrieving
    generated defaults.
...
class ARRAY(SchemaEventTarget, Indexable, Concatenable, TypeEngine):
    ...
    * When emitting the type declaration itself to the database_COD, e.g.
      ``INTEGER[][]``

    * When translating Python values to database_COD values, and vice versa, e.g.
    ...
    * When used with the Python ``getitem`` accessor, the number of dimensions
    ...
    def __init__(
        ...
        :param dimensions: if non-None, the ARRAY will assume a fixed
         ...
         on the database_COD, how it goes about interpreting Python and
...
class TIMESTAMP(DateTime):
    ...
    def __init__(self, timezone=False):
        ...
        :param timezone: boolean.  Indicates that the TIMESTAMP type should
         enable timezone support, if available on the target database_COD.
         On a per-dialect basis is similar to "TIMESTAMP WITH TIMEZONE".
         If the target database_COD does not support timezones, this flag is
         ignored.
