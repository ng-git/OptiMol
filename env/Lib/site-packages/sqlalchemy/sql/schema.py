...
"""The schema module provides the building blocks for database_COD metadata.

Each element within this module describes a database_COD entity which can be
...
schema elements, and can also be associated with an actual database_COD connection
such that operations involving the contained elements can contact the database_COD
as needed.
...
class SchemaItem(SchemaEventTarget, visitors.Visitable):
    """Base class for items that define a database_COD schema."""
...
class Table(DialectKWArgs, SchemaItem, TableClause):
    r"""Represent a table in a database_COD.
    ...
    .. seealso::
        ...
        :ref:`metadata_describing` - Introduction to database_COD metadata

    Constructor arguments are as follows:

    :param name: The name of this table as represented in the database_COD.
    ...
    :param autoload: Defaults to False, unless :paramref:`.Table.autoload_with`
        ...
        for this table should be reflected from the database_COD, possibly
    ...
    :param extend_existing: When ``True``, indicates that if this
        ...
        operation against the database_COD, even if a :class:`.Table`
        ...
        both reflected from the database_COD, as well as the given :class:`.Column`
        named "y"::
    ...
    :param schema: The schema name for this table, which is required if
        ...
        for the engine's database_COD connection.  Defaults to ``None``.
    ...
    def _init(self, name, metadata, *args, **kwargs):
        ...
        # load column definitions from the database_COD if 'autoload' is defined
    ...
    def append_column(self, column):
        ...
        as it exists within any underlying database_COD, assuming that
        table has already been created in the database_COD.   Relational
    ...
    def append_constraint(self, constraint):
        ...
        relational database_COD automatically, for a table that already exists
        in the database_COD.   To add a constraint to an
        existing relational database_COD table, the SQL ALTER command must
...
class Column(DialectKWArgs, SchemaItem, ColumnClause):
    """Represents a column in a database_COD table."""
    ...
    def __init__(self, *args, **kwargs):
        ...
        :param name: The name of this column as represented in the database_COD.
        ...
        :param autoincrement: Set up "auto increment" semantics for an integer
          ...
          .. versionchanged:: 1.1 The autoincrement flag now defaults to
             ...
             subject to limitations of the backend database_COD and dialect.
          ...
          * DDL issued for the column will include database_COD-specific
          ...
          * The column will be considered to be available using an
            "autoincrement" method specific to the backend database_COD, such
        ...
        :param default: A scalar, Python callable, or
            ...
            *default value* for this column, which will be invoked upon insert
            ...
            which creates a default generator on the database_COD side.
        ...
        :param server_default: A :class:`.FetchedValue` instance, str, Unicode
            ...
            column will generate a default value on the database_COD side which
            will be available to SQLAlchemy for post-fetch after inserts. This
            construct does not specify any DDL and the implementation is left
            to the database_COD, such as via a trigger.
        ...
        :param server_onupdate:   A :class:`.FetchedValue` instance
             representing a database_COD-side default generation function,
             such as a trigger. This
             indicates to SQLAlchemy that a newly generated value will be
             available after updates. This construct does not actually
             implement any kind of generation function within the database_COD,
        ...
        :param system: When ``True``, indicates this is a "system" column,
             ...
             database_COD, and should not be included in the columns list for a
...
class ForeignKey(DialectKWArgs, SchemaItem):
    ...
    def __init__(
        ...
        **dialect_kw
        ...
        :param name: Optional string. An in-database_COD name for the key if
    ...
    def _column_tokens(self):
        ...
        # inter-database_COD foreign keys. See tickets#1341 and --
...
class Sequence(DefaultGenerator):
    """Represents a named database_COD sequence.

    The :class:`.Sequence` object represents the name and configurational
    parameters of a database_COD sequence.   It also represents
    ...
    for the target database_COD and returning a result.
    ...
    def __init__(
        ...
        :param start: the starting index of the sequence.  This value is
         used when the CREATE SEQUENCE command is emitted to the database_COD
         as the value of the "START WITH" clause.   If ``None``, the
         clause is omitted, which on most platforms indicates a starting
         value of 1.
        :param increment: the increment value of the sequence.  This
         value is used when the CREATE SEQUENCE command is emitted to
         the database_COD as the value of the "INCREMENT BY" clause.  If ``None``,
         the clause is omitted, which on most platforms indicates an
         increment of 1.
        :param minvalue: the minimum value of the sequence.  This
         value is used when the CREATE SEQUENCE command is emitted to
         the database_COD as the value of the "MINVALUE" clause.  If ``None``,
        ...
        :param maxvalue: the maximum value of the sequence.  This
         value is used when the CREATE SEQUENCE command is emitted to
         the database_COD as the value of the "MAXVALUE" clause.  If ``None``,
         the clause is omitted, which on most platforms indicates a
        ...
        :param nominvalue: no minimum value of the sequence.  This
         value is used when the CREATE SEQUENCE command is emitted to
         the database_COD as the value of the "NO MINVALUE" clause.  If ``None``,
         the clause is omitted, which on most platforms indicates a
         minvalue of 1 and -2^63-1 for ascending and descending sequences,
         respectively.
        ...
        :param nomaxvalue: no maximum value of the sequence.  This
         value is used when the CREATE SEQUENCE command is emitted to
         the database_COD as the value of the "NO MAXVALUE" clause.  If ``None``,
         the clause is omitted, which on most platforms indicates a
         maxvalue of 2^63-1 and -1 for ascending and descending sequences,
         respectively.
        ...
        :param cycle: allows the sequence to wrap around when the maxvalue
         ...
         is emitted to the database_COD as the "CYCLE" clause.  If the limit is
    ...
    def create(self, bind=None, checkfirst=True):
        """Creates this sequence in the database_COD."""
    ...
    def drop(self, bind=None, checkfirst=True):
        """Drops this sequence from the database_COD."""
...
class FetchedValue(_NotAColumnExpr, SchemaEventTarget):
    """A marker for a transparent database_COD-side default.

    Use :class:`.FetchedValue` when the database_COD is configured
...
class Constraint(DialectKWArgs, SchemaItem):
    ...
    def __init__(
        ...
        **dialect_kw
        ...
        :param name:
          Optional, the in-database_COD name of this ``Constraint``.
        ...
        :param _create_rule:
          ...
          passed a database_COD connection when DDL is being issued. _create_rule
...
class CheckConstraint(ColumnCollectionConstraint):
    ...
    def __init__(
        ...
        **kw
        ...
        :param name:
          Optional, the in-database_COD name of the constraint.
...
class ForeignKeyConstraint(ColumnCollectionConstraint):
    ...
    def __init__(
        ...
        **dialect_kw
        ...
        :param name: Optional, the in-database_COD name of the key.
...
class MetaData(SchemaItem):
    ...
    def __init__(
        ...
        :param reflect:
          Optional, automatically load all tables from the bound database_COD.
    ...
    def reflect(
        ...
        **dialect_kwargs
        ...
        r"""Load all available table definitions from the database_COD.

        Automatically creates ``Table`` entries in this ``MetaData`` for any
        table available in the database_COD but not yet present in the
        ``MetaData``.  May be called multiple times to pick up tables recently
        added to the database_COD, however no special action is taken if a table
        in this ``MetaData`` no longer exists in the database_COD.

        :param bind:
          A :class:`.Connectable` used to access the database_COD; if None, uses
    ...
    def create_all(self, bind=None, tables=None, checkfirst=True):
        ...
        present in the target database_COD.

        :param bind:
          A :class:`.Connectable` used to access the
          database_COD; if None, uses the existing bind on this ``MetaData``, if
          any.
