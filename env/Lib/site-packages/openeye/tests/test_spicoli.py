...
# about this warning.
"""
 Test suite for Spicoli TK examples
"""
...
@pytest.mark.skipif(check_license('spicoli') is False, reason='SPICOLI license not found')
class TestSpicoli(OETestCase):

    def test_psa(self):
        script = self.run_example(
            '{0}/spicoli/psa.py'.format(self.EXAMPLES_DIR),
            '{0}/data.smi'.format(self.DATA_DIR))
        self.assertFalse(script.returncode)

    def test_calculate_triangle_areas(self):
        script = self.run_example(
            '{0}/spicoli/CalculateTriangleAreas.py'.format(self.DOCEXAMPLES_DIR),
            '{0}/data3d.sdf.gz'.format(self.DATA_DIR))
        self.assertFalse(script.returncode)

    def test_make_connected_surface_cliques(self):
        script = self.run_example(
            '{0}/spicoli/MakeConnectedSurfaceCliques.py'.format(self.DOCEXAMPLES_DIR),
            '{0}/data3d.sdf.gz'.format(self.DATA_DIR))
        self.assertFalse(script.returncode)

    def test_surface_data_access(self):
        script = self.run_example('{0}/spicoli/SurfaceDataAccess.py'.format(self.DOCEXAMPLES_DIR))
        self.assertFalse(script.returncode)
