...
# about this warning.
"""
 Test suite for Zap TK examples
"""
...
@pytest.mark.skipif(check_license('zap') is False, reason='ZAP license not found')
class TestZap(OETestCase):

    def test_zap_forces(self):
        script = self.run_example(
            '{0}/zap/zap_forces.py'.format(self.EXAMPLES_DIR),
            '{0}/data3d.sdf.gz'.format(self.DATA_DIR), expect_stderr=True)
