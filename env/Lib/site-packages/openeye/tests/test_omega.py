...
# about this warning.
"""
 Test suite for Omega TK examples
"""
...
@pytest.mark.skipif(check_license('omega') is False, reason='OMEGA license not found')
class TestOmega(OETestCase):

    def test_omega(self):
        script = self.run_example(
            '{0}/omega/simple_omega.py'.format(self.EXAMPLES_DIR),
            '{0}/data3d.sdf.gz'.format(self.DATA_DIR), 'simpleomega.oeb.gz',
            expect_stderr=True)
