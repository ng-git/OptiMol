...
class SessionTransaction(object):
    ...
    among multiple :class:`.Connection` objects, maintaining a database_COD
    ...
    **Life Cycle**
    ...
    with no database_COD connections.  As the :class:`.Session` is called upon
...
class Session(_SessionClassMethods):
    ...
    def __init__(
        ...
        :param autoflush: When ``True``, all query operations will issue a
           ...
           not be called repeatedly in order for database_COD queries to retrieve
        ...
        :param expire_on_commit:  Defaults to ``True``. When ``True``, all
           ...
           transaction will load from the most recent database_COD state.
        ...
        :param twophase:  When ``True``, all transactions will be started as
            ...
            of the database_COD in use along with an XID.  During a
            :meth:`~.commit`, after :meth:`~.flush` has been issued for all
            attached databases, the :meth:`~.TwoPhaseTransaction.prepare`
            method on each database_COD's :class:`.TwoPhaseTransaction` will be
            called. This allows each database_COD to roll back the entire
    ...
    def begin(self, subtransactions=False, nested=False):
        ...
        .. warning::
            ...
            started when the next database_COD operation is invoked.  See
        ...
        transactional state of the database_COD connection unless a rollback is
    ...
    def begin_nested(self):
        ...
        The target database_COD(s) and associated drivers must support SQL
    ...
    def commit(self):
        ...
        By default, the :class:`.Session` also expires all database_COD
        loaded state on all ORM-managed attributes after transaction commit.
        This so that subsequent operations load the most recent
        data from the database_COD.   This behavior can be disabled using
    ...
    def invalidate(self):
        ...
        the database_COD is known to be in a state where the connections are
    ...
    def no_autoflush(self):
        ...
        of objects which involve existing database_COD queries,
    ...
    def refresh(
        ...
        A query will be issued to the database_COD and all attributes will be
        refreshed with their current database_COD value.
        ...
        in database_COD state outside of that transaction - usage of
    ...
    def expire_all(self):
        ...
        in database_COD state outside of that transaction.
            ...
            if obj is not None:
                ...
                if (
                    ...
                    raise exc.FlushError(
                        ...
                        "auto-generated value, check that the database_COD table "
    ...
    def add(self, instance, _warn=True):
        ...
        Its state will be persisted to the database_COD on the next flush
        operation.
    ...
    def delete(self, instance):
        ...
        The database_COD delete operation occurs upon ``flush()``.
    ...
    def merge(self, instance, load=True):
        ...
        to load the object from the database_COD based on primary key, and if
        ...
        :param load: Boolean, when False, :meth:`.merge` switches into
         ...
         events as well as all database_COD access.  This flag is used for
         cases such as transferring graphs of objects into a :class:`.Session`
         from a second level cache, or to transfer just-loaded objects
         into the :class:`.Session` owned by a worker thread or process
         without re-querying the database_COD.
    ...
    def enable_relationship_loading(self, obj):
        ...
        will attempt to load a value from the database_COD using this
        ...
        **not** participate in any persistence operations; its state
        ...
        If the object instead represents an existing identity in the database_COD,
    ...
    def flush(self, objects=None):
        """Flush all the object changes to the database_COD.

        Writes out all pending object creations, deletions and modifications
        to the database_COD as INSERTs, DELETEs, UPDATEs, etc.  Operations are
        ...
        changes from Python to the database_COD's transaction buffer.
        ...
        try:
            ...
            if not objects and self.identity_map._modified:
                ...
                util.warn(
                    ...
                    "reset, and will not result in database_COD updates. "
    ...
    def is_modified(self, instance, include_collections=True, passive=None):
        ...
        * Instances present in the :attr:`.Session.dirty` collection may
          ...
          is the same as that loaded from the database_COD, resulting in no net
          change here.
        * Scalar attributes may not have recorded the previously set
          value when a new value was applied, if the attribute was not loaded,
          or was expired, at the time the new value was received - in these
          cases, the attribute is assumed to have a change, even if there is
          ultimately no net change against its database_COD value. SQLAlchemy in
    ...
    def is_active(self):
        ...
        or not actual database_COD connection resources are in use;  the
        :class:`.SessionTransaction` object coordinates among zero or more
        actual database_COD transactions, and starts out with none, accumulating
...
def make_transient(instance):
    ...
    .. warning::
        ...
        * were not present in the query which loaded this object, such as that
        ...
        As the object is transient and un-associated with any database_COD
